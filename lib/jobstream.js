...
import ConfigResolver from "../async-workers/configResolver.js";
import SqsClient from "./sqsClient.js";

class Jobstream {
    constructor({ workerId, config } = { workerId: null, config }) {
        this.client = new SqsClient({ queueName: config.queue_name });
        const log = (() => {
            if (workerId) {
    ...
    static async create({ workerId } = { workerId: null }) {
        ...
        const config = await ConfigResolver.config({
